...
## こだわりポイント/AIの使用方法
別ファイルにまとめています
- [こだわりポイント](docs/features.md)
- [AIの使用方法](docs/agentUsage.md)
...
- **UI**: Ant Design v6
- **データフェッチ**: Server Component から `fetch` を直接呼び出し / Next.js Data Cache
- **バリデーション**: zod
- **スタイリング**: Sass Modules
...
- **データフェッチは Server Component から `fetch` を直接呼び出し**（クライアント用フェッチライブラリは未使用）。キャッシュは `next.revalidate` / `next.tags` で制御
...
- **App Router 規約ファイル**（`error.tsx` / `loading.tsx` / `not-found.tsx`）を適切に配置
